...
export function AuthProviderConfig() {
  const { config, toggleProvider } = useAuthProvidersContext()
  ...
  return (
    <Card className="w-full max-w-md">
      <CardHeader>
        ...
        <CardDescription>
          Choose which authentication methods to show on the login page
        </CardDescription>
      </CardHeader>
      <CardContent>
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Button
              variant={config.google ? "default" : "outline"}
              size="sm"
              onClick={() => toggleProvider('google')}
            >
              {config.google ? 'Enabled' : 'Disabled'}
            </Button>
          </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <Button
              variant={config.github ? "default" : "outline"}
              size="sm"
              onClick={() => toggleProvider('github')}
            >
              {config.github ? 'Enabled' : 'Disabled'}
            </Button>
          </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="2">
                <path d="M4 7V4a2 2 0 0 1 2-2h8m-4 2v2"/>
            ...
            <Button
              variant={config.credentials ? "default" : "outline"}
              size="sm"
              onClick={() => toggleProvider('credentials')}
            >
